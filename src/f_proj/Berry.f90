...
SUBROUTINE Berry_proj2(l, eigval, O_bar, A_bar, dH_bar, berry)
  USE kinds, ONLY: DP
  USE constants, ONLY: cmplx_0, cmplx_i
  USE f_params, ONLY: dE_thr, E_fermi
  USE system, ONLY: Nw
  ...
  COMPLEX(DP), INTENT(IN)::dH_bar(Nw, Nw, 3)
  REAL(DP), INTENT(OUT)::berry(3)
  COMPLEX(DP):: D_val(Nw, Nw, 3), sum_val1(3), sum_val2(3)
  INTEGER::m, n, a, b, c, l
  DO m = 1, Nw
    DO n = 1, Nw
      IF (ABS(eigval(m) - eigval(n)) <= dE_thr) THEN
  ...
  berry = 0.0_DP
  ! IF (eigval(l) > E_fermi) RETURN
  sum_val1 = cmplx_0
  sum_val2 = cmplx_0
  DO m = 1, Nw
    IF (eigval(l) < E_fermi .AND. eigval(m) < E_fermi) CYCLE
    IF (eigval(l) > E_fermi .AND. eigval(m) > E_fermi) CYCLE
    DO c = 1, 3
      a = MOD(c, 3) + 1
      b = MOD(a, 3) + 1
      sum_val1(c) = sum_val1(c) &
                   + D_val(l, m, a)*A_bar(m, l, b) &
                   - D_val(l, m, b)*A_bar(m, l, a) !&
    END DO
    DO c = 1, 3
      a = MOD(c, 3) + 1
      b = MOD(a, 3) + 1
      sum_val2(c) = sum_val2(c) &
                    + cmplx_i*D_val(l, m, a)*D_val(m, l, b)
    END DO
  END DO
  IF (eigval(l) < E_fermi) THEN
    berry = berry + REAL(O_bar(l, l, :) - (sum_val1 + sum_val2), DP)
  ELSE
    berry = berry + REAL(-(sum_val1 + sum_val2), DP)
  END IF
  ! berry = berry + REAL(O_bar(l, l, :) - (sum_val1 + sum_val2), DP)
END SUBROUTINE Berry_proj2

SUBROUTINE Berry_proj3(l, eigval, O_bar, A_bar, dH_bar, berry)
  USE kinds, ONLY: DP
